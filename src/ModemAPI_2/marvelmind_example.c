...
static char *marvelmindDSPFilterString(uint8_t filterIndex)
{
    switch (filterIndex)
    {
    case MM_US_FILTER_19KHZ:
        return "19 kHz";
    case MM_US_FILTER_25KHZ:
        return "25 kHz";
    case MM_US_FILTER_31KHZ:
        return "31 kHz";
    case MM_US_FILTER_37KHZ:
        return "37 kHz";
    case MM_US_FILTER_45KHZ:
        return "45 kHz";
    default:
        return "unknown";
    }
...
bool marvelmindCheckSetRobotCommand(char *token1, char *token2, char *token3, char *token4, char *token5, char *token6)
{
    ...
    uint8_t address = atoi(token2);
    MarvelmindRobotCommand ms;
    ms.commandId = atoi(token3);

    ms.param1 = atoi(token4);
    ms.param2 = atoi(token5);
    ms.param3 = atoi(token6);

    if (mmSetRobotCommand(address, &ms))
    {
        printf("Command sent\r\n");
    }
    else
...
void marvelmindCycle()
{
    ...
    case waitPort:
    {
         if (mmOpenPortByName("/dev/ttyACM0"))
        //if (mmOpenPort())
        {
