...
void decouplante_Robot_controller(real_T rtu_x_ref, real_T rtu_y_ref, real_T
  ...
  DW_Robot_controller_decouplan_T *localDW)
{
  ...
  if (localDW->DiscreteTimeIntegrator_DSTATE > Vm) {
    localB->addantiwindup = Vm;
  } else if (localDW->DiscreteTimeIntegrator_DSTATE < -Vm) {
    localB->addantiwindup = -Vm;
  } else {
    localB->addantiwindup = localDW->DiscreteTimeIntegrator_DSTATE;
  }
  ...
  tmp = cos(rtu_theta);
  tmp_0 = sin(rtu_theta);
  *rty_Vg = -tmp_0 / localB->addantiwindup * rtb_Sum_o + tmp /
    localB->addantiwindup * rtb_Sum_l;
  ...
  *rty_Vg *= L / 2.0;
  ...
  /* Sum: '<S1>/Sum2' */
  *rty_Vd = localB->addantiwindup + *rty_Vg;

  /* Sum: '<S1>/Sum3' */
  *rty_Vg = localB->addantiwindup - *rty_Vg;
   ...
   */
  localDW->Integrator_DSTATE += I_t * rtb_Sum * SAMPLING_TIME;

  /* Update for DiscreteIntegrator: '<S28>/Filter' */
  localDW->Filter_DSTATE += SAMPLING_TIME * rtb_FilterCoefficient;

  /* Update for DiscreteIntegrator: '<S77>/Integrator' incorporates:
   *  Gain: '<S74>/Integral Gain'
   */
  localDW->Integrator_DSTATE_g += I_t * rtb_Sum1 * SAMPLING_TIME;

  /* Update for DiscreteIntegrator: '<S72>/Filter' */
  localDW->Filter_DSTATE_c += SAMPLING_TIME * rtb_FilterCoefficient_l;
}
