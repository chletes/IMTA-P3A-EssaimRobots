...
//////////////////////////////////////////////////////////////////////////////
HANDLE OpenSerialPort_ (const char * portFileName, uint32_t baudrate, bool verbose){
    HANDLE ttyHandle = CreateFile( TEXT(portFileName), GENERIC_READ|GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL);
...
static uint8_t receiveAnswerPacket(SERIAL_PORT_HANDLE ttyHandle, uint16_t accessMode, uint8_t *buffer, uint8_t packetSize){
    ...
    if (accessMode == MM_ACCESS_MODE_REMOTE){// In remote mode two replies should be received: first reply from modem, and second from remote device
...
static uint8_t checkReceivedPacket(uint8_t *buffer, uint8_t packetSize){
    ...
    if (buffer[1]&0x80){// if 7-th bit in byte 1 is set, it should be special 5-byte answer with code of error
        packetSize= 5;
    }
    ...
    if (buffer[1]&0x80){// return received code of error
...
static uint8_t getBeaconState(SERIAL_PORT_HANDLE ttyHandle, struct BeaconState* beaconState){
    ...
    if (!sendRequestPacket(ttyHandle, &buffer[0], sendPacketSize)){// failed to send request
